...
function downloadUrl(version, artifact) {
  return `https://github.com/howar31/dscrd/releases/download/v${version}/${artifact}`;
}

function checksumUrl(version) {
  return `https://github.com/howar31/dscrd/releases/download/v${version}/checksums.txt`;
}
...
async function main() {
  ...
  if (fs.existsSync(binPath) && fs.existsSync(versionFile)) {
    ...
    if (installed === version) {
      console.error(`dscrd v${version} is already installed, skipping.`);
      return;
    }
    console.error(`Upgrading dscrd from v${installed} to v${version}`);
  }

  if (fs.existsSync(INSTALL_DIR)) rmSync(INSTALL_DIR, { recursive: true, force: true });
  mkdirSync(INSTALL_DIR, { recursive: true });

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dscrd-"));
  ...
  try {
    ...
    console.error(`dscrd v${version} installed.`);
  } finally {
...
main().catch((err) => {
  console.error(`Error installing dscrd: ${err.message}`);
  process.exit(1);
});
