...
// keyed by path relative to the skills output directory (e.g. "dscrd/SKILL.md",
// "dscrd-shared/SKILL.md", "dscrd-channel/SKILL.md").
...
func renderGroupDirectory(root *cobra.Command) string {
	...
	for _, c := range visibleChildren(root) {
		...
		fmt.Fprintf(&b, "| `%s` | %s | [dscrd-%s](../dscrd-%s/SKILL.md) |\n",
...
// subcommands (e.g. `dscrd api`, `dscrd version`) for inclusion in the index.
...
func writeLeaf(b *strings.Builder, c *cobra.Command, level string) {
	...
	if m := c.Annotations["discordEndpoint"]; m != "" {
		fmt.Fprintf(b, "**Discord API:** `%s`\n\n", m)
	}
	if p := c.Annotations["perms"]; p != "" {
		fmt.Fprintf(b, "**Bot permissions:** `%s`\n\n", p)
	}
	if in := c.Annotations["intents"]; in != "" {
		fmt.Fprintf(b, "**Privileged intents:** `%s` (enable in Developer Portal -> Bot -> Privileged Gateway Intents)\n\n", in)
	}
	fmt.Fprintf(b, "```bash\n%s\n```\n\n", c.UseLine())
