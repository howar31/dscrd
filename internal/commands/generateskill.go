...
// metadata.version. Previously generated `dscrd`/`dscrd-*` dirs are removed
// first so a deleted command group leaves no stale skill behind.
...
// cleanGeneratedSkills removes the previously generated skill dirs (`dscrd` and
// `dscrd-*`) under dir, leaving any non-generated content untouched. A match is
// only removed when it is a directory containing a SKILL.md — so an unexpected
// --output-dir (e.g. the repo root) cannot delete the built `dscrd` binary or
// unrelated files/dirs that merely match the `dscrd`/`dscrd-*` pattern.
func cleanGeneratedSkills(dir string) error {
	matches, err := filepath.Glob(filepath.Join(dir, "dscrd"))
	if err != nil {
		return err
	}
	more, err := filepath.Glob(filepath.Join(dir, "dscrd-*"))
	if err != nil {
		return err
	}
	for _, d := range append(matches, more...) {
		...
		if err != nil || !info.IsDir() {
			continue // skip files (e.g. the dscrd binary)
		}
