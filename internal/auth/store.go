// Package auth manages dscrd credentials: profiles and token resolution.
package auth
...
// Load and re-encrypted by Save. ApplicationID and DefaultGuild are not
// sensitive and stay plaintext.
type Profile struct {
	Token         string `toml:"token,omitempty"`
	ApplicationID string `toml:"application_id,omitempty"`
	DefaultGuild  string `toml:"default_guild,omitempty"`
}

// Config is the on-disk dscrd configuration. KeyBackend records which backend
...
// defaultPath returns ~/.config/dscrd/config.toml.
func defaultPath() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".config", "dscrd", "config.toml"), nil
}

// ConfigPath returns the config path, honoring the DSCRD_CONFIG env override.
func ConfigPath() (string, error) {
	if p := os.Getenv("DSCRD_CONFIG"); p != "" {
		return p, nil
	}
	return defaultPath()
}
...
// decryptInPlace decrypts each enc:v1: token in cfg, best-effort: a field whose
// key is unavailable or whose ciphertext is invalid is left as-is (ResolveToken
// then reports it). Plaintext and empty fields are left untouched.
...
// Save encrypts the token of every profile and writes the config atomically
// with 0600 permissions. The caller's cfg is not mutated.
