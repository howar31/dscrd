...
// IsEncrypted reports whether v is still a ciphertext value. Exposed for
// callers that must skip an unusable token Load could not decrypt (key
// unavailable), e.g. `auth status`.
